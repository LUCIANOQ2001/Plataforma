...
class sesionController {
    ...
    public function add_sesion_controller(int $cursoId, array $post): string {
        ...
        if (!$titulo || !$fecha) {
            return '<div class="alert alert-warning">Complete título y fecha.</div>';
        }
        $stmt = $this->pdo->prepare("
        ...
        return '<div class="alert alert-success">Sesión creada.</div>';
    }

    // … tu método get_sesion_by_id_controller() …
    ...
    public function add_grabacion_controller(int $sesionId): string {
        if(empty($_FILES['grabacion']['name'])){
            return '<div class="alert alert-warning">Seleccione un archivo.</div>';
        }
        $file     = $_FILES['grabacion'];
        $ext      = pathinfo($file['name'], PATHINFO_EXTENSION);
        $allowed  = ['mp4','mkv','mp3','wav'];
        if(!in_array(strtolower($ext), $allowed)){
            return '<div class="alert alert-danger">Formato no permitido.</div>';
        }
        // guarda en carpeta /uploads/grabaciones/
        ...
        if(move_uploaded_file($file['tmp_name'], $dest)){
            ...
            return '<div class="alert alert-success">Grabación guardada.</div>';
        } else {
            return '<div class="alert alert-danger">Error al subir archivo.</div>';
        }
    }
    ...
    public function delete_grabacion_controller(int $id): string {
        ...
        if($row=$get->fetch(PDO::FETCH_ASSOC)){
            ...
            $del->execute([$id]);
            return '<div class="alert alert-success">Grabación eliminada.</div>';
        }
        return '<div class="alert alert-danger">No encontrado.</div>';
    }

    /** (Opcional) editar solo etiqueta o reemplazar archivo */
    public function update_grabacion_controller(int $id, string $newLabel): string {
        ...
        return '<div class="alert alert-success">Grabación actualizada.</div>';
    }

    // --- FORO ---
    /** Listar hilos de foro de una sesión */
    ...
    public function add_foro_controller(int $sesionId, array $post): string {
        ...
        return '<div class="alert alert-success">Foro creado.</div>';
    }

    /** Obtener datos de un hilo de foro */
    public function get_foro_by_id_controller(int $foroId): PDOStatement {
        $stmt = $this->pdo->prepare("
            SELECT id, sesion_id, Titulo, Descripcion, FechaSubida, FechaCierre
              FROM foro
             WHERE id = ?
        ");
        $stmt->execute([$foroId]);
    ...
    public function add_respuesta_controller(int $foroId, array $post, string $estCodigo): string {
        ...
        if(!$texto) {
            return '<div class="alert alert-warning">Escriba su respuesta.</div>';
        }
        $stmt = $this->pdo->prepare("
        ...
        return '<div class="alert alert-success">Respuesta enviada.</div>';
    }

    public function delete_respuesta_controller(int $id): string {
        $stmt = $this->pdo->prepare("DELETE FROM foro_respuesta WHERE id = ?");
        $stmt->execute([$id]);
        return '<div class="alert alert-success">Respuesta eliminada.</div>';
    }
}
